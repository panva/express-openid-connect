...
const { jwtVerify, createLocalJWKSet, decodeJwt } = require('jose');
...
class RequestContext {
  ...
  async fetchUserInfo() {
    ...
    return oidcClient.fetchUserInfo(
      configuration,
      ts.access_token,
      claims.sub
    );
...
class ResponseContext {
  ...
  async callback(options = {}) {
    ...
    try {
      ...
      try {
        ...
        // Check for OAuth error response in body - only handle early if no valid auth verification
        if (req.body && req.body.error && !authVerification) {
          const errorData = {
            error: req.body.error,
            error_description: req.body.error_description || req.body.error,
          };
          throw createError(400, req.body.error_description || req.body.error, errorData);
        }
  ...
  async backchannelLogout() {
    ...
    try {
      ...
      // Fetch JWKS from the issuer's jwks_uri
      const jwksResponse = await fetch(serverMetadata.jwks_uri);
      if (!jwksResponse.ok) {
      ...
      const { payload } = await jwtVerify(logoutToken, localJWKSet, {
